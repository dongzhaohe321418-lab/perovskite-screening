...
iodide-vacancy migration barrier, attributes every effect to a physical mechanism, and
...
interstitial dopants. The deliverables are (i) the first \emph{mechanism-annotated}
...
magnitude of the GA$^+$-induced $\Delta E_a$; (d) the strain--$E_a$ correlation
(tensile lowers, compressive raises).

\textbf{Objective 2 (High-throughput screening).} Compute mechanism-annotated
$\Delta E_a$ values for $\sim$50 candidate dopants spanning A-site substitution
(GA$^+$, dimethylammonium, acetamidinium), B-site substitution (Sr$^{2+}$, Ca$^{2+}$,
Bi$^{3+}$, La$^{3+}$) --- retained deliberately as a \emph{negative-control class}
anchored to the published null result of Arber et al.\ (2025) --- X-site substitution
(Cl$^-$, SCN$^-$), and interstitials (K$^+$, Rb$^+$, multivalent cations), each over
symmetry-inequivalent migration paths binned by dopant--vacancy distance.
...
Kohn--Sham DFT provides the ground-truth PES at $\mathcal{O}(N^3)$ cost. All production
cells use the room-temperature photoactive orthorhombic $\gamma$-phase of CsPbI$_3$
(consistent with Arber et al., 2025), whose broken symmetry is handled by explicit
enumeration of inequivalent hop directions. Four system-specific corrections are
...
before experimental hand-off. Method development is performed
first on the fully inorganic CsPbI$_3$ lattice to decouple methodology from the
organic-cation orientational degrees of freedom, which are subsequently sampled by
...
NEB barriers while confirming that MPtrj-trained models systematically
\emph{underestimate} barriers through PES softening (Deng et al., 2025).
...
and softening biases zero-shot barriers systematically low (Deng et al., 2025).
...
interstitial injection into $2\times2\times2$ $\gamma$-CsPbI$_3$ supercells;
symmetry-inequivalent configuration enumeration; generation of iodide-vacancy hop pairs
binned by dopant--vacancy distance (first shell, second shell, remote bulk reference).
...
3--4 & Objective 1 anchors: charge-state ordering vs Tyagi et al.; GA$^+$
$\Delta E_a$; strain--$E_a$ curve under imposed lattice strain & Four anchors
reproduced \\
5--7 & Phases 1--2 at scale: full candidate enumeration; MLIP-NEB farm ($\sim$2000
paths) with first-cycle fine-tuned models & Fine-tuned $\Delta E_a$ pre-screen \\
